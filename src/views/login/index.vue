<template lang="">
    <div class="login w-full h-full ">
        <div class="fixed top-1/2 bg-white shadow-md left-1/2 -translate-x-1/2 flex flex-col gap-8 -translate-y-1/2  px-10 py-10 rounded-md">
            <div class="flex">
                <div class="flex flex-col gap-2" @click="activeTab = 'Login'">登录</div>
                <div class="flex flex-col gap-2" @click="activeTab = 'Register'">注册</div>
            </div>
            ...
            <input type="text" class="border border-black px-2 rounded-md" placeholder="邮箱" v-if="activeTab === 'Register'" v-model="loginForm.email">
...
const handleLogin = () => {
    ...
    serverApi[activeTab.value](loginForm).then((res: any) => {
        console.log(res.data)
        activeTab.value === 'Login' && userStore.login(res.data)
        activeTab.value === 'Login' && router.push('/')
    })
}
